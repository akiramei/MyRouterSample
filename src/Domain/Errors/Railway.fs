...
module Railway =
    ...
    /// エラーをマップする
    let mapError (f: IError -> IError) (result: Result<'a>) : Result<'a> =
        match result with
        | Success value -> Success value
        | Failure error -> Failure(f error)

    /// 結果をフォールドする
    let fold (successFunc: 'a -> 'b) (failureFunc: IError -> 'b) (result: Result<'a>) : 'b =
        match result with
    ...
    let validate predicate error x =
        if predicate x then Success x else Failure error
