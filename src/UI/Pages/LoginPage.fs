...
module LoginPage =
    ...
    // IError表示コンポーネント
    [<ReactComponent>]
    let private ErrorView (error: IError option) =
        match error with
        | Some err ->
            let errorMsg = err.UserMessage
              ...
              Html.form
                  [ prop.onSubmit handleSubmit
                    prop.children
                        [
                          // ユーザー名入力
                          UsernameField model.Username model.Language (SetUsername >> dispatch)

                          // パスワード入力
                          PasswordField model.Password model.Language (SetPassword >> dispatch)

                          // 言語選択
                          LanguageSelector model.Language (SetLanguage >> dispatch)
                                      ...
                                      prop.children
                                          [ Daisy.card
                                                [ prop.className "shadow-xl bg-base-100"
                                                  prop.children [ Daisy.cardBody [ LoginCardContent model dispatch ] ] ] ] ] ] ] ] ]
